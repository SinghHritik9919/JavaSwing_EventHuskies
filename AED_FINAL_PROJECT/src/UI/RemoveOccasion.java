 ...
 * @author vivekmishra
 */
public class RemoveOccasion extends javax.swing.JPanel {
    ...
    public RemoveOccasion(String fName) {
        initComponents();
        this.fName = fName;
        populateReviewTable();
    ...
    private void initComponents() {
        ...
        reviewTable.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "EventID", "EventName", "Date", "City", "State", "Status"
            }
        ));
        ...
        jLabel2.setIcon(new javax.swing.ImageIcon(getClass().getResource("/UI/forms/delete.png"))); // NOI18N
    ...
    private void deleteEvent(String buttonName) {
        ...
        Occasions Occasion = (Occasions) reviewTableModel.getValueAt(selectedRow, 1);

        try ( Connection conn = DbUtility.getConnection();  
                PreparedStatement ps = conn.prepareStatement(IQueryMapper.DELETE_EVENT)) {

            ps.setString(1, Occasion.getEventId());
            ps.executeUpdate();
            ...
            Logger.getLogger(RemoveOccasion.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    List<Occasions> OccasionList;

    try (Connection conn = DbUtility.getConnection(); PreparedStatement ps = conn.prepareStatement(IQueryMapper.SELECT_EVENTS)) {
        ps.setString(1, this.fName);
        ResultSet rs = ps.executeQuery();
        if (!rs.next()) {
            JOptionPane.showMessageDialog(this, "Currently No Records to Display");
            return;  // Return to avoid processing an empty result set
        }

        OccasionList = new ArrayList<>();

        do {
            Occasions e = new Occasions();
            e.setEventId(rs.getString(1));
            e.setEventName(rs.getString(5));
            ...
            e.setCapacity(rs.getString(4));
            e.setStatus(rs.getString(14));
            e.setFromDate(rs.getString(8));
        ...
        for (Occasions prop : OccasionList) {
            Object[] row = new Object[6];

            row[0] = prop.getEventId();
            row[1] = prop;
            row[2] = prop.getFromDate();
            row[3] = prop.getCity();
            row[4] = prop.getState();
